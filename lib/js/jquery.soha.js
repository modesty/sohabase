...
(function (factory) {
    ...
    var _generic_error_msg = "Sorry, we're experiencing some issues right now, please try again later.";
    var _generic_error_service = "Sorry, we're in the middle of maintenance, please try again in 1 hour.";
    var SvcHelper = function(onSuccess, onError) {
        this.onSuccess = onSuccess;
        this.onError = onError;
    };

    SvcHelper.prototype.getXHRErrorMsg = function(xhr) {
        var errMsg = _generic_error_msg;
        try {errMsg = xhr.statusText;}catch(err) {errMsg = _generic_error_msg;}
        return errMsg;
    };
    SvcHelper.prototype.getXHRStatusCode = function(xhr) {
        var statusCode = 709;
        try {statusCode = xhr.status;}catch(e){statusCode = 709;}
        return statusCode;
    };
    SvcHelper.prototype.getResponseMsg = function(response){
        var errMsg = _generic_error_service;
        try {errMsg = response.status.message;}catch(e){errMsg = _generic_error_service;}
        return errMsg;
    };
    SvcHelper.prototype.getResponseCode = function(response){
        var errCode = 708;
        try {errCode = response.status.code;}catch(e){errCode = 708;}
        return errCode;
    };
    SvcHelper.prototype.isBadResponse = function (response) {
        var responseCode = this.getResponseCode(response);
        return (responseCode > 206 || responseCode < 200);
    };

    SvcHelper.prototype.onResponse = function(response, text, xhr) {
        if (this.isBadResponse(response)) {
            if (_.isFunction(this.onError)) {
                var errMsg = this.getResponseMsg(response);
                var errCode = this.getResponseCode(response);
                _.defer(this.onError, {message: errMsg, code: errCode});
            }
        }
        else {
    ...
    SvcHelper.prototype.onXHRError = function(xhr, textStatus, errorThrown) {
        if (_.isFunction(this.onError)) {
            var errMsg = this.getXHRErrorMsg(xhr);
            var errCode = this.getXHRStatusCode(xhr);
            _.defer(this.onError, {message:errMsg, code:errCode});
        }
    };
    ...
    cls.postJson = function (postURL, jsDataObj, onSuccess, onError, httpMethodOverride) {
        ...
        $.ajax({
            ...
            async:true,
            success:_.bind(svcHelper.onResponse, svcHelper), //Function( PlainObject data, String textStatus, jqXHR jqXHR )
            error:_.bind(svcHelper.onXHRError, svcHelper) //Function( jqXHR jqXHR, String textStatus, String errorThrown )
        });
    };

    cls.setSessionStorageItem = function(key, strVal) {
        //HTML5 sessionStorage doesn't persist across browser tabs
        //use $.cookie for cross-tab session data
        //use $.localStorage for cross-session non-expire data
    ...
    cls.getRequireJSConfig = function() {
        if (_.isObject(window.requirejs) && _.has(requirejs, 's')) {
